...
class Language(object):
    ...
    """
    def __init__(self, signs, fields, max_syns=2, 
            params=None
            ):
    ...
    def _add_link(self):
        ...
        if fields:
            targets = [c for c in self.concepts if self.concept2field[c] in fields]
    ...
    def count(self, basic):
        ...
        for idx in basic:
            ...
            if refs:
                ...
                selected = [x for x in refs_sorted[:3] if refs.count(x) > \
                        best_refs / 3] or [refs_sorted[0]]
            else:
                selected = []
            basics[idx] = selected
        return basics
...
class Phylogeny(object):
    ...
    def __init__(self, signs, fields, max_syns=2, params=None):
        ...
        self.language = Language(signs, fields, max_syns=max_syns, params=params)
        params = params or self.language.params
        if not 'change_range' in params:
    ...
    def prepare(self, taxa, change_range=None, logfile='phylo.log',
        ...
        self.tree = lingpy.basic.tree.Tree(lingpy.basic.tree.random_tree(taxa,
    ...
    def start(self, basic=None, verbose=True):
        ...
        for node in self.tree.preorder():
            if node.Name == 'root':
                ...
                new_language = self.tracer[node.Parent.Name]['language'].clone()
                distance = random.randint(self.params['change_min'], self.params['change_range'])
                if verbose: print('... analyzing node {0} ({1})'.format(node.Name, distance))
                ...
                self.tracer[node.Name] = dict(
                        ...
                        distance=distance)
                self.words += [w for w in new_language.words if w not in
                        self.words]
                if node.istip():
...
if __name__ == '__main__':
    ...
    for i in range(100):
        ...
        phy.prepare(taxa, change_range=2000,
                change_min=1900)
        ...
        wl.add_entries('cog2', 'concept,cogid', lambda x, y: str(x[y[0]]) + '-' + str(x[y[1]]))
        wl.renumber('cog2')
        ...
        d3 = phy.tree.get_distance(lingpy.basic.tree.Tree(lingpy.basic.tree.random_tree(taxa)),
                distance='rf')
        dists1 += [d1]
        dists2 += [d2]
        dists3 += [d3]
        ...
        wlen = len(phy.words)
        print('[i] generated tree {0} with distance of {1:.2f} vs. {2:.2f} vs.  {4:.2f} ({3:.2f}, {5}, {6:.2f}).'.format(
                    i+1,
                    d1,
                    d2,
                    adist,
